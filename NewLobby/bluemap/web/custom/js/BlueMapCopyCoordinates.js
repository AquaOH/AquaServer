...
window.copyText = function (data, mode) {
  let textToCopy;
  switch (mode) {  // 使用传入的 mode 而不是全局的 copyMode
    case 1:
...
window.bluemap.popupMarker.onMapInteraction = function(evt) {
  ...
  button.addEventListener("click", () => copyText(copyMode === 3 ? {key, value: data} : data, copyMode));  // 传递 copyMode
